...
def elasticQuerySkel():
    ...
    res = es.search(index="logstash-bluecoat-v4-*", body={"query": {"match_all": {}}},size=100)
    ipList = []
    print "[*] Querying Elasticsearch for non domains !!!"
...
def whoisFinder(*tmpIpList):
    tmp = []
    tmp = list(tmpIpList)
    proxy = urllib2.ProxyHandler({'http': '10.10.2.141:8080'})
